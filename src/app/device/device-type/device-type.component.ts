...
export class DeviceTypeComponent {
  ...
  use(id: any) {
    this.rs.get(this.url+`device/type/${id}/enable`).subscribe((res) => {
      this.load();
    });
  }
  forbid(id: any) {
    this.rs.get(this.url+`device/type/${id}/disable`).subscribe((res) => {
      this.load();
    });
  }
  edit(id: any) {
    const path = `type/edit/${id}`;
    this.router.navigateByUrl(path);
  }
  cancel() {
    this.msg.info('取消操作');
